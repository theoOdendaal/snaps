...
// a cli flag for this?
pub fn handle_list(display_size: bool, select_indexes: Option<Vec<usize>>, select_tags: Option<Vec<Tags>>) -> Result<(), Error> {
    ...
    for i in 0..snapshot_count {
        ...
        if is_selected {
            write!(writer, "\x1b[{};{}m", Colour::Yellow.fg_code(), Colour::Blue.bg_code())?;
        } else if is_latest {
            write!(writer, "\x1b[{}m", Colour::Cyan.fg_code())?;
        }

        let reset_code = if is_selected || is_latest { "\x1b[0m" } else { "" };
...
pub fn epoch_to_datetime(secs: u64) -> (i32, u32, u32, u32, u32, u32) {
    ...
    // Time component 
    let (hours, remaining_secs)  = ((time_secs / HOUR_IN_SECONDS) as u32, (time_secs % HOUR_IN_SECONDS) as u32); 
